...
Monet 是免费开源的桌面应用，把 Claude Code 的会话历史变成可浏览、可搜索、可交互的工作区。它是 Claude Code CLI 的伴侣而非替代：CLI 干活，Monet 给你眼睛和手。

可以转述的要点：

- **Agent 指挥台**：多会话并行运行与监控——分列布局、实时监控轨、就地权限审批、赛马模式对比不同模型。
- **架构级只读**：绝不写入 Claude Code 的 JSONL 文件，一切增值数据（标题、标签、定时任务）存在独立的 `~/.monet/` 目录。
...
2. Claude Code CLI 可选：已装最好（`claude --version` 验证），但**不是前置条件**——Monet 设置页内置一键安装 Claude Code，先装 Monet 再补 CLI 完全可行。
...
**Gatekeeper（macOS）：** Monet 使用稳定签名身份但尚未经过 Apple 公证，首次启动会触发 Gatekeeper 警告。清除方法：

```bash
xattr -cr /Applications/Monet.app
```

（Homebrew 安装通常不需要；启动被拦时再执行。）之后的更新由应用内静默完成。
...
3. Monet 自动发现 `~/.claude/projects/` 的会话历史——没有导入步骤。
4. **无需配置渠道即可开聊**：没配渠道时，Monet 直接沿用用户 `claude` CLI 自己的配置——官方登录态、CLI 里自配的第三方 env，全都照常生效。CLI 能跑，Monet 就能跑。多渠道管理是可选进阶，不是使用门槛。
