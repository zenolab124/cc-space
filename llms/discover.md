...
- Engine Center reports installation, authentication, version, capabilities, and diagnostics per engine; Codex checks for stable updates and shared model-cache compatibility and presents them in engine details without persistent notification dots. Different standalone and ChatGPT-bundled runtime versions are informational; one engine failure does not block another
...
- Free-form drag arrangement: drag session cards to reorder, drag column headers to swap, drag dividers to resize (hold Shift to resize all columns together), drag cards between monitor rail and columns
