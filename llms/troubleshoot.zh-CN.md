...
**从 1.x 升级后权限型功能集体失灵**（在终端恢复、通知、屏幕观察、定时任务授权等）。2.x 起签名身份升级为 Developer ID，macOS 按签名记账的系统权限（自动化、辅助功能、屏幕录制、完全磁盘等）会整体视为未授权，这是一次性成本而非 bug。修复：打开 设置 → 权限体检，按面板逐项重新授权——主应用与后台任务 Runner 两本账都要授；若系统设置的对应面板里存在旧的 Monet / monet-routine-runner 条目，先删除旧条目再重新授权，避免勾选到失效记录。菜单栏与小组件若消失，在「设置 → 菜单栏」重新批准后台项目即可。
