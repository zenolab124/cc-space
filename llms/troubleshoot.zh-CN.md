...
**从 1.x 升级后权限型功能集体失灵**（在终端恢复、通知、屏幕观察、定时任务授权等）。2.x 起签名身份升级为 Developer ID，macOS 按签名记账的系统权限（自动化、辅助功能、屏幕录制、完全磁盘等）会整体视为未授权，这是一次性成本而非 bug。修复：打开 设置 → 权限体检，按面板逐项重新授权——主应用与签名的后台 Helper `MonetRoutineRunner.app` 有各自账本。Runner 的自动化、辅助功能、屏幕录制、完全磁盘访问可用面板里的「重建 Runner 授权」精确重置；Local Network 归因主应用且不能由 `tccutil` 精确重置，应走面板的显式检测/授权与应用副本治理。菜单栏与小组件若消失，在「设置 → 菜单栏」重新批准后台项目即可。

**启动被拦（无法验证开发者）。** 正式版已经过 Apple 公证，正常不会被拦。若被拦，先确认版本：早期未公证版本升级到最新版即可解决；若最新版仍被拦，可到「系统设置 → 隐私与安全性」找到 Monet 的提示并点「仍要打开」完成一次性授权（该按钮在尝试打开后才出现），同时按下方「提 bug」流程报告——公证版被拦属于异常。不要运行 `xattr` 移除系统安全属性。
...
**定时任务不执行。** Routine 由 launchd 执行（`io.github.zenolab124.monet.routine.<id>`），执行体是主应用 bundle 内已签名的后台 Helper `MonetRoutineRunner.app`，不是数据目录中的裸 runner。自动化、辅助功能、屏幕录制与完全磁盘访问按 Helper 身份记账；Local Network 通过 launchd 的关联 bundle 归因主应用。打开 设置 → 权限体检，经真实 launchd 链路检查；需要重建 Runner 授权时使用面板按钮，不要手工猜测或删除系统记录。

**权限型功能静默失效**（在终端恢复、UI 自动化、屏幕观察、本地网络）。打开设置 → 权限体检：逐项显示已授/未授与修复方法。Runner 的四类可重置权限交给面板精确重建；Local Network 没有受支持的 `tccutil` 精确重置服务，只能显式再探测、在系统设置中授权，并处理同 bundle ID 的重复应用副本。不要建议全局 `tccutil reset`，它会清掉无关授权且仍解决不了 Local Network 历史记录。
