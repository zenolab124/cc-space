...
describe('feature integration boundaries', () => {
  it('targets ordinary workbenches through a split button and remembers the last one', () => {
    ...
    expect(button).toContain("existing ? 'i-carbon-launch' : 'i-carbon-add'")
  ...
  it('keeps semantic copy modes distinct and sanitizes enhanced HTML', () => {
    ...
    const globalStyles = source('../../src/styles/extends.css')
    ...
    expect(semanticCopy).toContain('selection.collapseToEnd()')
    expect(semanticCopy).not.toContain('selection.collapse(surface, 0)')
    expect(semanticCopy).toContain('selectionSpansSessionSurfaces(selection)')
    expect(semanticCopy).toContain("document.querySelectorAll<HTMLElement>('.session-surface')")
    expect(semanticCopy).toContain('rangeIndex < selection.rangeCount')
    expect(semanticCopy).toContain('touchedSurfaces.size > 1')
    expect(semanticCopy).toContain("documentElement.classList.add('selection-paint-reset')")
    expect(semanticCopy).toContain("documentElement.classList.remove('selection-paint-reset')")
    expect(globalStyles).toContain('html.selection-paint-reset .session-surface *')
    ...
    expect(semanticCopy).toContain("document.addEventListener('pointermove', onPointerMove, true)")
    expect(semanticCopy).toContain("document.removeEventListener('pointermove', onPointerMove, true)")
    expect(semanticCopy).toContain("document.addEventListener('pointerup', onPointerUp, true)")
    expect(semanticCopy).toContain("document.removeEventListener('pointerup', onPointerUp, true)")
    expect(semanticCopy).toContain('eventSessionSurface(event.target) !== pointerSelectionSurface')
