...
describe('feature integration boundaries', () => {
  it('targets ordinary workbenches through a split button and remembers the last one', () => {
    ...
    expect(sessionList).toContain('variant="secondary"')
    expect(sessionList).toContain('\n                compact\n')
    const sessionDetail = source('../../src/components/SessionDetail.vue')
    const readonlyBar = source('../../src/components/session/SessionReadonlyBar.vue')
    expect(sessionDetail).toContain('<WorkbenchTargetButton\n')
    expect(sessionDetail).toContain('v-if="effectiveSessionId"')
    expect(sessionDetail).toContain(':session-id="effectiveSessionId"')
    expect(readonlyBar).toContain('flex flex-wrap items-center')
    expect(readonlyBar).toContain('max-w-full flex-wrap items-center justify-end')
