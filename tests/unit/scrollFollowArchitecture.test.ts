...
describe('scroll follow race guards', () => {
  ...
  it('revalidates standard-engine timeline follow after nextTick and animation frame', () => {
    ...
    expect(wheel).toContain('hasUpwardScrollRange(element)')
    expect(wheel).toMatch(/event\.deltaY < 0[\s\S]*invalidateTimelineScrollRequests\(\)[\s\S]*scheduleTimelineFollowAfterUpwardIntent\(intentAt\)/)
    expect(wheel).not.toMatch(/event\.deltaY < 0[\s\S]{0,500}stopTimelineFollow\(\)/)
    expect(wheel).toContain('requestAnimationFrame(() =>')
    expect(wheel).toContain('resumeTimelineFollowIfAtBottom(element, intentAt)')
    expect(scroll).toContain('shouldDetachScrollFollowAfterMovement')
    expect(scroll).toContain('const previousScrollTop = lastTimelineScrollTop')
    expect(scroll).toContain('previousScrollTop,')
    ...
    expect(engine).toContain('timelineUpwardIntentAt > timelineDownwardIntentAt')
    expect(engine).toContain('scheduleTimelineFollowAfterUpwardIntent(intentAt)')
    ...
    expect(request).toContain('shouldDetachScrollFollowAfterMovement')
    expect(request).toContain('previousScrollTop: scheduledScrollTop')
  ...
  it('follows same-record streaming growth through ResizeObserver without direct watcher scroll writes', () => {
    ...
    expect(engine).toMatch(/watch\(\(\) => props\.session\.id,[\s\S]{0,520}resetTimelineFollow\(\)/)
