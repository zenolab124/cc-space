...
describe('scroll follow race guards', () => {
  ...
  it('revalidates standard-engine timeline follow after nextTick and animation frame', () => {
    ...
    expect(scroll).toContain('hasUpwardScrollRange(element)')
    ...
    expect(request).toContain('hasUpwardScrollRange(element)')
    expect(request).toContain('performance.now() - timelineUpwardIntentAt <= TIMELINE_SCROLL_INTENT_MS')
